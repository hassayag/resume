...
function Background() {
    ...
    useEffect(() => {
        ...
        camera = new THREE.OrthographicCamera(
            -width * DIMENSION_SCALER,
            width * DIMENSION_SCALER,
            height * DIMENSION_SCALER,
            -height * DIMENSION_SCALER
        );
        ...
        for (let i = -AMOUNT; i < AMOUNT; i++) {
            for (let j = -AMOUNT; j < AMOUNT; j++) {
                createCube(
                    new THREE.Vector3(i * (BOX_LENGTH + SPACING), 0, j * (BOX_LENGTH + SPACING)),
                    getRandomColor()
                );
            }
        }
...
function onWindowResize() {
    ...
    (camera.left = -width * DIMENSION_SCALER), (camera.right = width * DIMENSION_SCALER);
